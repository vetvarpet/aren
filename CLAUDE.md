...
- New game checklist: portal card (+cover CSS class, ՆՈՐ tag, DICT ×10), trophies row + `REAL` names ×11, about.html timeline ×11 + game count words, README row, mood-filter tag in `MOODS`, **and an entry in `time/index.html`**.
- **`time/index.html` is the site's diary and must never fall behind.** Every notable day — new games, upgrades, deletions, team changes — gets a dated `.ev` entry (use `.ev big` + a `.tag` for milestones). Aren checks this page and has issued a formal reprimand for letting it go stale. Update it in the same commit as the change, not later.
